#!/bin/bash
# @critical: openclaw-morning-briefing.sh から呼ばれる（間接的に launchd com.openclaw.morning-briefing）
# @stops-if-deleted: 朝ブリーフィングの本文生成が失敗。フォールバックメッセージだけが届くようになる
# @depends: scan-projects.sh, task-engine.js, sns-weekly-draft.js
...
# --- 期限データは Notion Task DB に統合済み（today.json に含まれる） ---
# deadlines.json は廃止。task-engine.js が Notion Task DB から直接取得する。
...
### タスク情報
※ 期限・タスクは全てNotion Task DBが正本（task-engine.jsが取得済み、today.jsonに含まれる）
※ 期限超過タスクもtoday.jsonのurgent枠に含まれている
