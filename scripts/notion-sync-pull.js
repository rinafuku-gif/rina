 ...
 *   3. Obsidian ダッシュボード.md の「## 直近のタスク」セクション
 *   4. Obsidian 02_プロジェクト/タスク一覧.md（全体再生成）
 *
 * GTD矛盾スキャン:
...
const DASHBOARD = path.join(VAULT, "ダッシュボード.md");
const TASK_LIST = path.join(VAULT, "02_プロジェクト", "タスク一覧.md");
...
function renderDashboardSection(groups, issues) {
  const line = (t, withDate = true) => {
    const tag = t.project || "未分類";
    const datePart = withDate && t.date ? `（${fmtMD(t.date)}）` : "";
    return `- [ ] **${tag}**: ${t.title}${datePart}`;
  };
  const doneLine = (t) => `- [x] **${t.project || "未分類"}**: ${t.title}`;

  let out = "## 直近のタスク\n\n";
  const totalIssues = (issues?.staleMaybe?.length || 0) + (issues?.shouldPromote?.length || 0) + (issues?.overdue?.length || 0);
  if (totalIssues > 0) {
    out += `> ⚠️ **GTD矛盾 ${totalIssues}件検出** — Notionで修正してください（期限超過:${issues.overdue.length} / 昇格候補:${issues.shouldPromote.length} / 期日あり+いつかやるかも:${issues.staleMaybe.length}）\n\n`;
  }
  out += "### 期限超過\n\n";
  out += groups.overdue.length ? groups.overdue.map((t) => line(t)).join("\n") + "\n\n" : "（なし）\n\n";
  out += "### 今週やること\n\n";
  out += groups.thisWeek.length ? groups.thisWeek.map((t) => line(t)).join("\n") + "\n\n" : "（なし）\n\n";
  out += "### 進行中\n\n";
  out += groups.inProgress.length ? groups.inProgress.map((t) => line(t, false)).join("\n") + "\n\n" : "（なし）\n\n";
  out += "### 完了（直近7日）\n\n";
  out += groups.recentDone.length ? groups.recentDone.map(doneLine).join("\n") + "\n\n" : "（なし）\n\n";
  out += `<!-- auto-generated by notion-sync-pull.js at ${new Date().toISOString()} -->\n\n`;
  return out;
}

// ---- Dashboard: replace a single top-level section by heading ----
function replaceSection(content, heading, newSection) {
  const lines = content.split("\n");
  let startIdx = lines.findIndex((l) => l.trim() === heading);
  if (startIdx === -1) return content; // heading not found, no-op
...
// ---- Dashboard write (multi-section replacement) ----
function writeDashboard(tasksSection, urgentSection) {
  if (!fs.existsSync(DASHBOARD)) {
    log(`Dashboard not found: ${DASHBOARD}`);
    return;
  }
  let content = fs.readFileSync(DASHBOARD, "utf8");
  // 冒頭に "## 直近のタスク" がなければ追加
  if (!/^## 直近のタスク/m.test(content)) {
    content = tasksSection + content;
  } else {
    content = replaceSection(content, "## 直近のタスク", tasksSection);
  }
  // "## 今すぐ注意が必要なこと" があれば差し替え（なければそのまま）
  content = replaceSection(content, "## 今すぐ注意が必要なこと", urgentSection);
  atomicWrite(DASHBOARD, content);
}

// ---- Urgent section (replaces manual "## 今すぐ注意が必要なこと") ----
function renderUrgentSection(groups) {
  // 🔴 期限超過 OR 今日・明日 / 🟡 2-7日以内 / 🟢 8-14日以内
  const rows = [];
  for (const t of groups.overdue) {
    rows.push({ priority: "🔴", t });
  }
  for (const t of groups.thisWeek) {
    const d = daysBetween(t.date);
    if (d <= 1) rows.push({ priority: "🔴", t });
    else if (d <= 7) rows.push({ priority: "🟡", t });
  }
  // 8-14日先のtasks（グループ外から探す）
  // グループには含まれないが、アクティブで今週外のものを見る必要はなし。
  // 代わりに進行中のものも入れる
  const seen = new Set(rows.map((r) => r.t.id));
  for (const t of groups.inProgress) {
    if (!seen.has(t.id)) rows.push({ priority: "🟡", t });
  }
  ...
  out += "|--------|------|------|------|\n";
  for (const r of top) {
    const title = r.t.title.replace(/\|/g, "\\|");
    const tag = r.t.project || "—";
    const dateStr = r.t.date ? fmtMD(r.t.date) : "期日なし";
    out += `| ${r.priority} | **${title}** | ${dateStr} | ${tag} |\n`;
  }
  out += "\n→ 全タスク: [[タスク一覧]] / 補助金: [[補助金一覧]]\n\n";
  out += `<!-- auto-generated by notion-sync-pull.js at ${new Date().toISOString()} -->\n\n`;
  return out;
}

// ---- Task list write (full regen) ----
function writeTaskList(groups, allActive) {
  const byTag = {};
  ...
  let out = "# タスク一覧\n\n";
  out += `> Notion Task DB の自動生成ビュー（${new Date().toISOString()}）\n> 編集はNotionで行ってください。このファイルは5分ごとに上書きされます。\n\n`;
  out += "## GTDステータス別\n\n";
  out += `- **期限超過**: ${groups.overdue.length}\n`;
  out += `- **今週やること**: ${groups.thisWeek.length}\n`;
  out += `- **進行中**: ${groups.inProgress.length}\n`;
  out += `- **完了（直近7日）**: ${groups.recentDone.length}\n\n`;
  out += "## 事業タグ別（アクティブ）\n\n";
  for (const tag of Object.keys(byTag).sort()) {
    out += `### ${tag}\n\n`;
    for (const t of byTag[tag].sort((a, b) => (a.date || "9999").localeCompare(b.date || "9999"))) {
      const datePart = t.date ? `（${fmtMD(t.date)}）` : "";
      const gtdBadge = t.gtd ? `\`${t.gtd}\` ` : "";
      out += `- [ ] ${gtdBadge}${t.title}${datePart}\n`;
    }
    out += "\n";
  }
  fs.mkdirSync(path.dirname(TASK_LIST), { recursive: true });
  atomicWrite(TASK_LIST, out);
}
...
(async () => {
  ...
  try {
    ...
    writeDashboard(renderDashboardSection(groups, issues), renderUrgentSection(groups));
    writeTaskList(groups, active);
